...
# 모델 로드
model = tf.keras.models.load_model('model/rnn_LSTM_pred_mentalHealth.h5')
...
# 사용자 입력 받기
heart_rate = st.number_input("평균 심박수 (bpm)", min_value=40, max_value=150, value=70)
sleep_hours = st.number_input("하루 평균 수면 시간 (시간)", min_value=0.0, max_value=15.0, value=7.0)
...
if st.button("예측하기"):
    ...
    prediction_label = "정신 건강 양호" if prediction > 0.5 else "정신 건강 나쁨"
